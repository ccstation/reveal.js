...
(function( root, factory ) {
	...
	var DEFAULT_SLIDE_SEPARATOR = '^\r?\n---\r?\n$',
		...
		DEFAULT_SECTION_ATTRIBUTES_SEPARATOR = '\\\.section:\\\s*?(\\\S.+?)$',
	...
	function slidify( markdown, options ) {
		...
		for( var i = 0, len = sectionStack.length; i < len; i++ ) {
			// vertical
			sectionAttr = getAttributeFromPattern(sectionStack[i], DEFAULT_SECTION_ATTRIBUTES_SEPARATOR);
			
			if( sectionStack[i] instanceof Array ) {
				markdownSections += '<section '+ options.attributes + ' ' + sectionAttr + '>';
	...
	function getAttributeFromPattern( array, separator ) {
		var mardownClassesSectionRegex = new RegExp( separator, 'mg' );
		var mardownClassRegex = new RegExp( "([^\"= ]+?)=\"([^\"=]+?)\"", 'mg' );
		
		var output = "";
		if( array instanceof Array ) {
			array.forEach( function( child ) {
				if( matches = mardownClassesSectionRegex.exec( child ) ) {
					
					while( matchesClass = mardownClassRegex.exec( matches[1] ) ) {
						output = output + " " + matchesClass[1] + "='" + matchesClass[2] + "'";
					}
				}
			} );
		}
